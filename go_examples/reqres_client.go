...
func sendmessageInit() error {
	...
	//msg := startString + strings.Repeat("0", querySize-len(startString))
	fmt.Println("INFO:",msg)
	temp, err := stream.Write([]byte(msg))
...
//}
func clientSender() {
	if stream == nil || time.Since(startTime) >= runTime{
		stream.Close()
	}else {
		err := sendmessageInit()
		if err != nil {
...
func clientMain(multipath bool) error {
	...
	//sess, err := quic_go.DialAddr(
	//	fmt.Sprintf("localhost:%d", 4242),
	//	getTLSClientConfig(),
	//	getQuicConfigForClient(&quic.Config{MaxIdleTimeout: idleTimeout}),
	//)
	session, err := quic_go.DialAddr(addr, tlsConfig, cfgClient)
	if err != nil {
		panic(err)
	}
